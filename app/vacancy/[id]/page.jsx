...
export default async function VacancyArticle({ params }) {
  ...
  return (
    <div className="flex items-start justify-center py-5 min-page-h gap-5">
      ...
      <aside className="flex flex-col justify-center p-2 rounded-md bg-white gap-2 shadow-lg">
        <h3>Компания</h3>
        ...
        <p className="w-min whitespace-break-spaces">{vacancyItems.profile}</p>
        <p className="w-min whitespace-break-spaces">{vacancyItems.name}</p>
      </aside>
    </div>
  );
}
